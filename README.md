...
### 0-1. (선택) 폴더 이름만 정리: `rename-folders`

`organize`와 별개로, **이미 `YYYYMMDD_설명` 형식으로 만들어둔 폴더 이름**을
`YYYY-MM-DD_설명`으로만 바꿔줍니다. 파일은 전혀 건드리지 않고 폴더 이름만
바뀝니다. 지정한 폴더 아래 모든 하위 폴더를 재귀적으로 훑습니다.

```
20120114_무주리조트     -> 2012-01-14_무주리조트
20120114-16_무주리조트  -> 2012-01-14-16_무주리조트   (뒤에 붙은 범위는 그대로 보존)
```

```powershell
photomanager rename-folders "F:\00. image\2012"
photomanager rename-folders "F:\00. image\2012" --apply
```

#### 명령어 없이 쓰기 (.bat)

저장소의 `scripts\rename-folders.bat`을 더블클릭하면 됩니다.
- 정리할 **폴더를 이 파일 아이콘 위에 끌어다 놓고 놓으면** 그 폴더로 바로 미리보기가 뜹니다.
- 그냥 더블클릭만 했다면 폴더 경로를 직접 입력하라는 안내가 나옵니다.
- 미리보기를 보여준 뒤 `y`를 입력해야 실제로 이름이 바뀝니다(`n` 또는 그냥 엔터면 취소).

바탕화면에 두고 쓰고 싶으면 `scripts\rename-folders.bat` 파일을 복사해서 바탕화면에 붙여넣으시면 됩니다.

이미 `YYYY-MM-DD` 형식인 폴더는 다시 안 건드리므로 반복 실행해도 안전하고,
바꾸려는 이름이 이미 존재하면 덮어쓰지 않고 건너뜁니다.
